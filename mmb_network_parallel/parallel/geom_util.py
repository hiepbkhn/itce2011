...
class Query:
    def __init__(self, obj_id, x=0, y=0, timestamp=0, next_node_x=0, next_node_y=0, 
                 next_node_id=0, cur_edge_id=0, k_anom=0, min_length=0):      
        self.obj_id = obj_id
        self.x = x
        self.y = y
...
class EdgeSegmentSet:
    ...
    #######################################################
    def clean_fixed_expanding(self, result):
        new_result = result
...
#                item.start_x, item.start_y, item.end_x, item.end_y)    
    ...
    #######################################################
    def parse_query_log(self, timestamp, query_file_name):
        trajs = {}
        f = open(option.QUERY_PATH + query_file_name[0:-4] + "_t" + str(timestamp) + ".txt", "r+") 
        ...
        while True:
            ...
            trajs[obj_id] = Query(obj_id, x, y, timestamp, next_node_x, next_node_y, 0, 0, k_anom, min_length)
    ...
    def parse_expanding_list(self):
        ...
        while True:
            ...
            #
            line = map.readline()
            seg_list_str = line.split(":")
            seg_list_str.pop()
            seg_list = []
            
            for seg_line in seg_list_str:
                items = seg_line.split(",")
                start_x = float(items[0])
                start_y = float(items[1])
                end_x = float(items[2])
                end_y = float(items[3])
                cur_edge_id = int(items[4])
                seg_list.append(EdgeSegment(start_x, start_y, end_x, end_y, cur_edge_id))
            
            expanding_list[obj_id] = seg_list
    ...
    #######################################################
    # 0: discard, 1:positive, 2:negative 
    def find_clique_type(self, clique, trajs, expanding_list, total_map_len):
        if len(clique) == 1:
            return 0
        #
        max_min_length = 0
...
#            query_list.append(query)
        ...
        mesh = []
        for obj_id in clique:
#            mesh = self.union(mesh, expanding_list[obj_id])
            # NEW (trial)
            mesh.extend(expanding_list[obj_id])
        # NEW (trial)
        mesh = self.clean_fixed_expanding(mesh)    


        clique_len = self.length(mesh)
    ...
    ####################################################### 
    # Dec, 31      
    def find_positive_negative_cliques(self, timestamp, total_map_len, 
                                               clique_group, query_log, expanding_list):
        positive_mc_set = []
        negative_mc_set = []
...
#                mesh = self.union(mesh, expanding_list[obj_id])
    ...
    def union(self, set_1, set_2):   
        ...
        return self.clean_fixed_expanding(result)
        
    
    #######################################################        
    def length(self, set_1):
        total_len = 0
        for item in set_1:
            total_len += math.sqrt((item.start_x - item.end_x)**2 + (item.start_y - item.end_y)**2)  
                                                        # EdgeSegment.length(item)
...
#        print found, mid
    ...
    #######################################################
    def show_sys_time(self, prompt):
        print prompt, datetime.now()  
