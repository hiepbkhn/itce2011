...
Jan-22: use 'multiprocessing' for finding positive/negative cloaking sets
'''
...
def mapped_mem_find_positive_negative_cliques(*args, **kwargs):
    
    timestamp, total_map_len, edge_segment_set, clique_group, query_file_name = args
    #
    positive_mc_set = []
    negative_mc_set = []
...
#        clique_groups.append(graph.mc_set[end:num_cliques]) 
        ...
        pool = mp.Pool(processes = num_groups)
        results = []
        
        for i in xrange(0, len(clique_groups)):               # nr of tasks to do > nr processes
            args = (timestamp, total_map_len, edge_segment_set, clique_groups[i], option.QUERY_FILE)                     # wrap arguments for worker function
            kwargs = {'name' : 'process ' + str(i)}   # keyword arguments (optional)
            results.append(pool.apply_async(mapped_mem_find_positive_negative_cliques, args, kwargs))   # add to pool
        ...
        num_positive = 0
        num_negative = 0
        for i in xrange(0, len(clique_groups)):
#            print(results[i].get(timeout = 1))   # print the result    
            res = results[i].get(timeout = 1)
            for clique in res[0]:
