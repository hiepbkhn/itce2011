...
int main(int argc, char* argv[]) {
	...
	Try
		...
		const TInt NZero = Env.GetIfArgPrefixInt("-n0:", 2,
				"Initiator matrix size");
		const TStr InitMtx = Env.GetIfArgPrefixStr("-m:", "0.9 0.7; 0.5 0.2",
				"Init Gradient Descent Matrix (R=random)").GetLc();
		const TInt GradIter = Env.GetIfArgPrefixInt("-gi:", 5,
				"Gradient descent iterations for M-step");
		const TInt EMIter = Env.GetIfArgPrefixInt("-ei:", 30, "EM iterations");
		const TFlt LrnRate = Env.GetIfArgPrefixFlt("-l:", 1e-5,
				"Learning rate");
		const TFlt MnStep = Env.GetIfArgPrefixFlt("-mns:", 0.001,
				"Minimum gradient step for M-step");
		const TFlt MxStep = Env.GetIfArgPrefixFlt("-mxs:", 0.008,
				"Maximum gradient step for M-step");
		const TInt WarmUp = Env.GetIfArgPrefixInt("-w:", 8000,
				"Samples for MCMC warm-up");
		const TInt NSamples = Env.GetIfArgPrefixInt("-s:", 2000,
		...
		const TBool Debug = Env.GetIfArgPrefixBool("-debug:", false,
				"Debug mode");
		...
		if (OutFNm.Empty()) {
			OutFNm = TStr::Fmt("%s-kronem%d", InFNm.GetFMid().CStr(), NZero());
		}
		...
		KronLL.RunKronEM(EMIter, GradIter, LrnRate, MnStep, MxStep, WarmUp,
				WarmUp, NSamples);

		const TKronMtx& FitMtx = KronLL.GetProbMtx();
		FILE *F = fopen(
				TStr::Fmt("KronEM-%s.tab", InFNm.GetFMid().CStr()).CStr(),
				"at");
		fprintf(F, "Input\t%s\n", InFNm.CStr());
		TStrV ParamV;
