	...
	for (int i = 0; i < LNames.Len(); i++) {
		for (int e = 0; e < Exts.Len(); e++) {
			for (int d = 0; d < 2; d++) {
				...
				// Draw new graph
				if (d) {
					TSnap::DrawGViz(NGraph1, TGVizLayout(i), FNameDemo,
							LNames[i], true);
				} else {
					TSnap::DrawGViz(UNGraph1, TGVizLayout(i), FNameDemo,
							LNames[i], true);
				}
...
void DrawGVizColorDemo() {
	...
	for (TUNGraph::TNodeI NI = UNGraph1->BegNI(); NI < UNGraph1->EndNI();
			NI++) {
	...
	for (int i = 0; i < LNames.Len(); i++) {
		for (int e = 0; e < Exts.Len(); e++) {
			for (int d = 0; d < 2; d++) {
				...
				TStr FName = TStr::Fmt("%s/demo_%s_%s_col.%s", DIRNAME,
						d ? "ngraph" : "ungraph", LNames[i].CStr(),
						Exts[e].CStr());
				...
				if (d) {
					TSnap::DrawGViz(NGraph1, TGVizLayout(i), FName, LNames[i],
							true, NIdColorH);
				} else {
					TSnap::DrawGViz(UNGraph1, TGVizLayout(i), FName, LNames[i],
							true, NIdColorH);
				}
				printf("Drawing graph '%s'\n", FName.CStr());
			}
...
int main(int argc, char* argv[]) {
	_mkdir(DIRNAME); //, S_IRWXU | S_IRWXG | S_IRWXO);
