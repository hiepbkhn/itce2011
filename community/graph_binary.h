...
public:
	...
	inline pair<vector<unsigned int>::iterator, vector<float>::iterator> neighbors(
			unsigned int node);
};
...
inline double Graph::nb_selfloops(unsigned int node) {
	...
	pair<vector<unsigned int>::iterator, vector<float>::iterator> p = neighbors(
			node);
...
inline pair<vector<unsigned int>::iterator, vector<float>::iterator> Graph::neighbors(
		unsigned int node) {
	assert(node >= 0 && node < nb_nodes);
