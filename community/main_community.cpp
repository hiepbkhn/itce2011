...
void usage(char *prog_name, const char *more) {
	...
	cerr << "usage: " << prog_name
			<< " input_file [-w weight_file] [-p part_file] [-q epsilon] [-l display_level] [-v] [-h]"
			<< endl << endl;
	cerr << "input_file: file containing the graph to decompose in communities."
			<< endl;
	cerr
...
//	srand(time(NULL) + getpid());
	...
	do {
		if (verbose) {
			...
			cerr << "  network size: " << c.g.nb_nodes << " nodes, "
					<< c.g.nb_links << " links, " << c.g.total_weight
					<< " weight." << endl;
		}
		...
		if (verbose)
			cerr << "  modularity increased from " << mod << " to " << new_mod
					<< endl;
	...
	if (verbose) {
		display_time("End");
		cerr << "Total duration: " << (time_end - time_begin) << " sec."
				<< endl;
	}
	cerr << new_mod << endl;
}
