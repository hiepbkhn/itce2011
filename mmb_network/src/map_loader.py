...
class VisitedEdge:
    def __init__(self, cur_edge_id, start_x, start_y):
...
class Stack:
    def __init__(self):
        self.stack = []
        self.visited = []   #list of visited edges 
        self.max_size = 0
    ...
    # 1: before, -1: after, 0: equal    
    def compare_visited(self, edge_1, edge_2):
        if edge_1.cur_edge_id < edge_2.cur_edge_id:
            return 1
        if edge_1.cur_edge_id > edge_2.cur_edge_id:
            return -1
        
        if (edge_1.start_x == edge_2.start_x and edge_1.start_y == edge_2.start_y):
            return 0 
        if (edge_1.start_x < edge_2.start_x) or (edge_1.start_x == edge_2.start_x and edge_1.start_y < edge_2.start_y):
            return 1
        else:
            return -1    
    ...
    def push(self, seg_item):
        ...
        insert_visited_loc = 0
        edge = VisitedEdge(seg_item.cur_edge_id, seg_item.x, seg_item.y)
        if len(self.visited) > 0:
            found_visited = False
            lo = 0
            hi = len(self.visited) - 1
            mid = (lo + hi) / 2
            
            while True:
                if self.compare_visited(self.visited[mid], edge) == 0:
                    found_visited = True
                    break
                if self.compare_visited(self.visited[mid], edge) == -1:
                    hi = mid - 1
                    if hi < lo: 
                        break
                else:
        ...
        self.stack.append(seg_item)
        self.visited.insert(insert_visited_loc, edge)
...
#        self.max_size = max(self.max_size, len(self.stack))        
    ...
    def print_all(self):
        ...
        for seg in self.stack:
            print seg.cur_edge_id, seg.x, seg.y, seg.end_x, seg.end_y, seg.is_node, seg.length
        print "VISITED"
        for edge in self.visited:
            print edge.cur_edge_id, edge.start_x, edge.start_y    
...
#        print "compute Interval Trees (X,Y): DONE"
    ...
    #######################################################
    def is_full_edge(self, seg):
        edge = self.edges[seg.cur_edge_id]
        start_node = self.nodes[edge.start_node_id]
        end_node = self.nodes[edge.end_node_id]
        if seg.start_x == start_node.x and seg.start_y == start_node.y and \
            seg.end_x == end_node.y and seg.end_y == end_node.y:
            return True
        if seg.start_x == end_node.x and seg.start_y == end_node.y and \
            seg.end_x == start_node.y and seg.end_y == start_node.y:
            return True
        return False
...
#                result.append(point)
        ...
        result = []
        for item in item_list:
            nodes.append((item.start_x, item.start_y, item.cur_edge_id))   
            nodes.append((item.end_x, item.end_y, item.cur_edge_id))
        nodes = sorted(nodes)
        
        for i in range(len(nodes)-1):
            if (nodes[i+1][0] != nodes[i][0]) or (nodes[i+1][1] != nodes[i][1]):
                result.append(nodes[i])
        if (nodes[-1][0] != nodes[-2][0]) or (nodes[-1][1] != nodes[-2][1]):
...
#    for radius in [300,400,500,600,700,800,900,1000,1100,1200,1300,1400,1500]:
#        print "radius =", radius
#        start = time.clock()
#        #8545.43, 16095.95, 98307178 - 8299, 7790, 940009
...
#    print "DONE 3"
    ...
    #TEST 3-2: EdgeSegmentSet.intersect()
#    result_1 = map_data.compute_fixed_expanding(8545.43, 16095.95, 98307178, 600)
#    result_1 = EdgeSegmentSet.clean_fixed_expanding(result_1)
#    
#    result_2 = map_data.compute_fixed_expanding(8381, 16296, 98307178, 300)
#    result_2 = EdgeSegmentSet.clean_fixed_expanding(result_2)
...
#        print "%15d %8.2f %10.2f %10.2f %10.2f %10.2f" % (item.cur_edge_id, EdgeSegment.length(item), \
#            item.start_x, item.start_y, item.end_x, item.end_y)
#    result = EdgeSegmentSet.intersect(result_1, result_2)
#    print "intersect"
#    for item in result:
#        print "%15d %8.2f %10.2f %10.2f %10.2f %10.2f" % (item.cur_edge_id, EdgeSegment.length(item), \
#            item.start_x, item.start_y, item.end_x, item.end_y)
