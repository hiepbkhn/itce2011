...
Nov-20: generate_cloaked_region_for_new_query()
+ use [last_query]
+ use delta_t * MAX_SPEED 
...
#        print i_list        
    ...
    #######################################################
    def find_init_cloaking_sets(self):
        ...
        for clique in self.mc_set:
            ...
            query_list = []
            for obj_id in clique:
                query = self.query_log.trajs[obj_id][0]
    ...
    def init_mc_set(self):
        ...
        for query in query_list:
            self.last_query[query.obj_id] = self.query_log.trajs[query.obj_id][0]
        ...
        for i in range(len(query_list)):
            for j in range(i+1,len(query_list)):
                if get_distance(query_list[i].x, query_list[i].y, query_list[j].x, query_list[j].y) <= option.INIT_DISTANCE:
                    num_edges += 1
    ...
    #######################################################
    def find_cloaking_set_for_new_query(self, user_u):
        
        can_cr = []             #list of cliques that contain user_u
        #1.
        for clique in self.mc_set:
            if len(clique) > 1 and (user_u in clique):
                can_cr.append(clique)
                
        #2. sort can_cr
        can_cr = sorted(can_cr, key=lambda clique:len(clique), reverse=True)
                
        #3.
        for clique in can_cr:
            max_min_area = 0
            max_k_anom = 0
            min_k_anom = 1000
            query_list = []
            for obj_id in clique:
                query = self.last_query[obj_id]
                query_list.append(query)
                if max_min_area < query.min_area:
                    max_min_area = query.min_area
                if max_k_anom < query.k_anom:
                    max_k_anom = query.k_anom
                if min_k_anom > query.k_anom:
                    min_k_anom = query.k_anom
                    
            mbr = CloakingSet.compute_mbr(query_list)
            #
            if len(clique) >= max_k_anom and mbr.area >= max_min_area:
                return clique              #POSITIVE 
            elif len(clique) < max(query.k_anom, min_k_anom) or mbr.area < max_min_area:
                return None                #NOT-CANDIDATE 
            else:                          #NEGATIVE --> convert (heuristically)
                found = False
                #sort
                query_list = sorted(query_list, key=lambda query: query.k_anom)
                ...
                while True:
                    ...
                    max_min_area = max(query_list, key=lambda query: query.min_area).min_area
                    mbr = CloakingSet.compute_mbr(query_list)
                    if len(query_list) >= query_list[-1].k_anom and mbr.area >= max_min_area:
                        found = True
                        break
                #    
                if found == True:
    ...
    #######################################################
    def generate_cloaked_region_for_new_query(self, user_u, clique):
        pass
