...
class BasicTests(unittest.TestCase):
    ...
    def testDeleteVertices(self):
        g = igr.Graph([(0,1), (1,2), (2,3), (0,2), (3,4), (4,5)])
        ...
        self.assertRaises(igr.InternalError, g.delete_vertices, 2)
    ...
    def testGraphGetEids(self):
        ...
        self.assertRaises(igr.InternalError, g.get_eids, pairs=[(0,1), (0,2)])

    def testAdjacency(self):
        g=igr.Graph(4, [(0,1), (1,2), (2,0), (2,3)], directed=True)
        ...
        self.failUnless(g.get_adjlist(igr.IN) == [[2], [0], [1], [2]])
        self.failUnless(g.get_adjlist(igr.ALL) == [[1,2], [0,2], [0,1,3], [2]])
        
    def testEdgeIncidency(self):
        g=igr.Graph(4, [(0,1), (1,2), (2,0), (2,3)], directed=True)
        self.failUnless(g.incident(2) == [2, 3])
        self.failUnless(g.incident(2, igr.IN) == [1])
        self.failUnless(g.incident(2, igr.ALL) == [2, 3, 1])
        self.failUnless(g.get_inclist() == [[0], [1], [2,3], []])
        self.failUnless(g.get_inclist(igr.IN) == [[2], [0], [1], [3]])
        self.failUnless(g.get_inclist(igr.ALL) == [[0,2], [1,0], [2,3,1], [3]])


    def testMultiplesLoops(self):
        g=igr.Graph.Tree(7, 2)
        ...
        g=igr.Graph(2, [(0,1), (1,0)], directed=True)
...
class GraphDictListTests(unittest.TestCase):
    ...
    def testGraphFromDictList(self):
        g = igr.Graph.DictList(self.vertices, self.edges)
        self.checkIfOK(g, "name")
        g = igr.Graph.DictList(self.vertices, self.edges, iterative=True)
        self.checkIfOK(g, "name")

    def testGraphFromDictIterator(self):
        g = igr.Graph.DictList(iter(self.vertices), iter(self.edges))
        self.checkIfOK(g, "name")
        g = igr.Graph.DictList(iter(self.vertices), iter(self.edges), iterative=True)
        self.checkIfOK(g, "name")

    def testGraphFromDictIteratorNoVertices(self):
        g = igr.Graph.DictList(None, iter(self.edges))
        self.checkIfOK(g, "name", check_vertex_attrs=False)
        g = igr.Graph.DictList(None, iter(self.edges), iterative=True)
        self.checkIfOK(g, "name", check_vertex_attrs=False)
    ...
    def testGraphFromDictListAlternativeName(self):
        for vdata in self.vertices:
            ...
            del vdata["name"]
        g = igr.Graph.DictList(self.vertices, self.edges, vertex_name_attr="name_alternative")
...
class GraphTupleListTests(unittest.TestCase):
    ...
    def testGraphFromTupleList(self):
        g = igr.Graph.TupleList(self.edges)
        self.checkIfOK(g, "name", ())

    def testGraphFromTupleListWithEdgeAttributes(self):
        g = igr.Graph.TupleList(self.edges, edge_attrs=("friendship", "advice"))
        self.checkIfOK(g, "name", ("friendship", "advice"))
        g = igr.Graph.TupleList(self.edges, edge_attrs=("friendship", ))
        self.checkIfOK(g, "name", ("friendship", ))
        g = igr.Graph.TupleList(self.edges, edge_attrs="friendship")
        self.checkIfOK(g, "name", ("friendship", ))

    def testGraphFromTupleListWithDifferentNameAttribute(self):
        g = igr.Graph.TupleList(self.edges, vertex_name_attr="spam")
        self.checkIfOK(g, "spam", ())

    def testGraphFromTupleListWithWeights(self):
        g = igr.Graph.TupleList(self.edges, weights=True)
        self.checkIfOK(g, "name", ("weight", ))
        g = igr.Graph.TupleList(self.edges, weights="friendship")
        self.checkIfOK(g, "name", ("friendship", ))
        g = igr.Graph.TupleList(self.edges, weights=False)
        self.checkIfOK(g, "name", ())
        self.assertRaises(ValueError, igr.Graph.TupleList,
                [self.edges], weights=True, edge_attrs="friendship")

    def testNoneForMissingAttributes(self):
        g = igr.Graph.TupleList(self.edges, edge_attrs=("friendship", "advice", "spam"))
...
class DegreeSequenceTests(unittest.TestCase):
    ...
    def testIsGraphicalSequence(self):
        self.assertTrue(igr.is_graphical_degree_sequence([]))
        self.assertTrue(igr.is_graphical_degree_sequence([], []))
        self.assertTrue(igr.is_graphical_degree_sequence([0]))
        self.assertTrue(igr.is_graphical_degree_sequence([0], [0]))
        self.assertFalse(igr.is_graphical_degree_sequence([1]))
        self.assertTrue(igr.is_graphical_degree_sequence([1], [1]))
        self.assertFalse(igr.is_graphical_degree_sequence([2]))
        self.assertFalse(igr.is_graphical_degree_sequence([2, 1, 1, 1]))
        self.assertTrue(igr.is_graphical_degree_sequence([2, 1, 1, 1], [1, 1, 1, 2]))
        self.assertFalse(igr.is_graphical_degree_sequence([2, 1, -2]))
        self.assertFalse(igr.is_graphical_degree_sequence([2, 1, 1, 1], [1, 1, 1, -2]))
        self.assertTrue(igr.is_graphical_degree_sequence([3, 3, 3, 3, 3, 3, 3, 3, 3, 3]))
        self.assertTrue(igr.is_graphical_degree_sequence([3, 3, 3, 3, 3, 3, 3, 3, 3, 3], None))
        self.assertFalse(igr.is_graphical_degree_sequence([3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3]))
        self.assertTrue(igr.is_graphical_degree_sequence([3, 3, 3, 3, 3, 3, 3, 3, 3, 3],
            [4, 3, 2, 3, 4, 4, 2, 2, 4, 2]))
        self.assertTrue(igr.is_graphical_degree_sequence([3, 3, 3, 3, 4]))
