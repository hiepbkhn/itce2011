%%%
% Nov 24, 2015
%   - normalize and pad distArr (computed in UtilityMeasure.java)
% Nov 25
%   - add e_cutArr   
function [ rel_err, e_degArr, e_distArr, e_cutArr, e_AD, e_MD, e_DV, e_CC, e_PL, e_APD, e_CL, e_EDiam, e_Diam ] = aa_compute_util(graph_name, sample_name, n_samples)
...
sum_cutArr = zeros(size(a_cutArr,1),1);
sum_AD = 0;
sum_MD = 0;
sum_DV = 0;
sum_CC = 0;
...
sum_cutArr = sum_cutArr/n_samples;
...
e_cutArr = norm(double(sum_cutArr-a_cutArr),1)/sum(double(a_cutArr));
...
% % for TmF table
% e_degArr = norm(sum_degArr-a_degArr,1);
% e_distArr = norm(sum_distArr-a_distArr,1);
% e_AD = sum_AD;
% e_MD = sum_MD;
% e_DV = sum_DV;
% e_CC = sum_CC;
