...
% 
% [rel_err_sg1k, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~] = aa_compute_util_all('../polbooks', '../polbooks_sg1k', 20, eps_str, '_4');  
%             
...
%     eps_arr, rel_err_dendro_4, '+:', eps_arr, rel_err_fixed_4, '*--', eps_arr, rel_err_sg1k, '^-');  % , eps_arr, rel_err_hrgdiv_4, 's--'
% axis([1 7.1 0 0.7]) 
...
[rel_err_sg1k, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~] = aa_compute_util_all('../polblogs-wcc', '../polblogs-wcc_sg1k', 20, eps_str, '_10');  
...
% 
% figure;
% plot(eps_arr, rel_err_1k, 's-', eps_arr, rel_err_tmf, 'o-', eps_arr(5:7), rel_err_ef, 'x-', eps_arr, rel_err_fixed_4, '*--'); % , eps_arr, rel_err_hrgdiv_4, 's--'      %axis([0 40 0 1])  
...
% 
% figure;
% plot(eps_arr, rel_err_1k, 's-', eps_arr, rel_err_tmf, 'o-', eps_arr(5:7), rel_err_ef, 'x-', eps_arr, rel_err_fixed_4, '*--'); % , eps_arr, rel_err_hrgdiv_4, 's--'      %axis([0 42 0 1])  
% % legend('1K','TmF','EF','HRG-Fixed');        % ,'HRG-Div'
% set(gcf,'units','points','position',[x0,y0,width,height])
% set(gca,'FontSize', 12)
% ylabel('rel.err')
