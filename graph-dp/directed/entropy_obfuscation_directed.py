...
Jul 10
    - add compute_eps_deterministic_multi()
Jan 14 2015    
...
def compute_deg_index(G):
    ...
    for deg_u in G.out_degree().itervalues():
    ...
    deg_index = {}
    i = 0
    for deg_u in deg_dict.iterkeys():
        deg_index[deg_u] = i
        i += 1
    #
...
#        print adj_list
        ...
        # first neighbor (edge)
        p = G.edge[v][adj_list[0]]['p']         
        d[0][0] = 1.0 - p
        d[0][1] = p
        
        for l in range(2,len(adj_list)+1):
            p = G.edge[v][adj_list[l-1]]['p']
...
def compute_eps(G, G2, k):
    max_deg_G2 = max(G2.out_degree().itervalues())
    print "max_deg_G2 =", max_deg_G2
    deg_list = G.out_degree()   # G not G2
    deg_list_G2 = G2.out_degree()     
...
#                print "len(EC) =", len(EC)
        ...
        for i in range(len(unique_edge_list)):
            ...
            if val < q:
                ...
                while True:
                    r_e = random.gauss(0,unique_edge_list[i][2])    # normal distribution
...
#    G.add_edges_from([(0,1,{'p':0.7}),(0,2,{'p':0.9}),(0,3,{'p':0.8}),(1,2,{'p':0.8}),(1,3,{'p':0.1})])
...
#    scipy.io.savemat("C:/" + file_name + "__deg_list.mat", dict(node_list=np.array(list(deg_list.iterkeys())), deg_list=np.array(list(deg_list.itervalues()))))
...
#    (eps_min, G_min) = generate_obfuscation(G, sigma=1.0, k=50, eps=0.01, c=2, q=0.01)
    ...
    out_file = "../out/" + file_name + "_entropy_01_2_001.out"
    nx.write_edgelist(G_min, out_file, '#', '\t', True, 'utf-8')
    
    # post-processing attack: rounding
    out_file = "../out/" + file_name + "_entropy_post_01_2_001.out"
