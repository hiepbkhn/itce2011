...
def compute_deg_index(G):
    ...
    for deg_u in nx.degree(G).itervalues():
...
#    G.add_edges_from([(0,1,{'p':0.7}),(0,2,{'p':0.9}),(0,3,{'p':0.8}),(1,2,{'p':0.8}),(1,3,{'p':0.1})])
    ...
    G.add_nodes_from([0,1,2,3])
    G.add_edges_from([(0,1,{'p':0.3}),(0,2,{'p':0.8}),(0,3,{'p':0.9}),(1,2,{'p':0.7}),(2,3,{'p':0.4})])

#    print nx.nodes(G)
#    print nx.degree(G)
    
    deg_list = nx.degree(G)
    X = compute_X(G, 4, 3, deg_list)
    Y = compute_Y(X, 4, 3, deg_list)
    ent = compute_entropy(Y, 4, 3, deg_list)
